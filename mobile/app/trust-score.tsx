...
interface TrustData { trust_score: number; tier: string; completed_orders: number; average_rating: number; completion_rate: number; on_time_percentage: number; }
...
export default function TrustScoreScreen() {
  ...
  useEffect(() => {
    if (user) api.get<ApiResponse<TrustData>>(`/erranders/${user.id}/trust-score`).then(({ data: d }) => setData(d.data));
  }, [user]);
...
const styles = StyleSheet.create({
  ...
  scoreValue: { fontSize: 42, fontWeight: 'bold', color: colors.neutral[600] },
  scoreMax: { fontSize: 16, color: colors.neutral[400] },
  tierBadge: { paddingHorizontal: 20, paddingVertical: 8, borderRadius: 20 },
  tierText: { fontSize: 18, fontWeight: 'bold' },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center' },
  metricCard: { width: '45%', backgroundColor: colors.white, borderRadius: theme.radius.lg, padding: 20, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
  metricValue: { fontSize: 28, fontWeight: 'bold', color: colors.primary[500], marginBottom: 4 },
  metricLabel: { fontSize: 13, color: colors.neutral[400], textAlign: 'center' },
  empty: { textAlign: 'center', color: colors.neutral[400], fontSize: 16, marginTop: 40 },
});
