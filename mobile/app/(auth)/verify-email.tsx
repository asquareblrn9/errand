...
export default function VerifyEmailScreen() {
  ...
  const registeredEmail = useAuthStore((s) => s.registeredEmail);
  ...
  const handleVerify = async (fullCode: string) => {
    ...
    try {
      await authService.verifyEmail(fullCode, registeredEmail ?? undefined);
      await loadUser();
      router.replace('/(tabs)');
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.desc}>
        ...
        <Text style={styles.email}>{registeredEmail || 'your email'}</Text>
      </Text>
...
const styles = StyleSheet.create({
  ...
  email: { fontWeight: '600', color: colors.neutral[600] },
  ...
  backLink: { textAlign: 'center', color: colors.neutral[400], fontSize: 14, marginTop: 24 },
});
